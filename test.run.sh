...
COMMANDS=('whoami' 'php -v' 'php-fpm -v' 'composer -V')
NGINX_COMMANDS=('nginx -v' 'nginx -t' 'supervisord version')
QUALITY_COMMANDS=('phan -v' 'phpcpd -v' 'phpcs --version' 'phpcbf --version' 'php-cs-fixer -V' 'phpmd --version' 'phpstan -V' 'phpunit --version')

set -x

for cmd in "${COMMANDS[@]}"; do docker run ${DOCKER_IMAGE} ${cmd}; done
for cmd in "${COMMANDS[@]}"; do docker run -e ASUSER=0 ${DOCKER_IMAGE} ${cmd}; done
